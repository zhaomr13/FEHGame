...
func _initialize():
    ...
    # Check key nodes exist in the scene file (prefix match: node headers may
    # carry a unique_id attribute since Godot 4.5)
...
func _test_army_visibility():
    ...
    army._ready()
    assert(not army.get_node("BorderPanel").visible, "Idle army in city should hide body")

    army.state = ArmyScript.ArmyState.PLANNED
    army.update_visibility()
    assert(not army.get_node("BorderPanel").visible, "Planned army still in city should hide body")

    army.state = ArmyScript.ArmyState.MOVING
    army.update_visibility()
    assert(army.get_node("BorderPanel").visible, "Moving army should show body")

    army.state = ArmyScript.ArmyState.IDLE
    army.current_city_id = ""
    army.update_visibility()
    assert(army.get_node("BorderPanel").visible, "Idle army not in a city should show body")
